...
def get_response(history, temperature, top_p, stream, selected_model):
    headers = {
    ...
    if stream:
        ...
        timeout = timeout_all
    response = requests.post(API_URL, headers=headers, json=payload, stream=True, timeout=timeout, proxies=proxy, verify=False)
    return response

def stream_predict(history, inputs, chatbot, all_token_counts, top_p, temperature, selected_model):
    ...
    try:
        response = get_response(history, temperature, top_p, True, selected_model)
...
def predict_all(history, inputs, chatbot, all_token_counts, top_p, temperature, selected_model):
    ...
    try:
        response = get_response(history, temperature, top_p, False, selected_model)
    except requests.exceptions.ConnectTimeout:
        status_text = standard_error_msg + connection_timeout_prompt + error_retrieve_prompt
        return chatbot, history, status_text, all_token_counts
...
def predict(history, inputs, chatbot, all_token_counts, top_p, temperature, stream=False, selected_model = MODELS[0], use_websearch_checkbox = False, should_check_token_count = True):  # repetition_penalty, top_k
    ...
    if stream:
        logging.info("使用流式传输")
        iter = stream_predict(history, inputs, chatbot, all_token_counts, top_p, temperature, selected_model)
        for chatbot, history, status_text, all_token_counts in iter:
            yield chatbot, history, status_text, all_token_counts
    else:
        logging.info("不使用流式传输")
        chatbot, history, status_text, all_token_counts = predict_all(history, inputs, chatbot, all_token_counts, top_p, temperature, selected_model)
        yield chatbot, history, status_text, all_token_counts
    ...
    if sum(all_token_counts) > max_token and should_check_token_count:
        ...
        yield chatbot, history, status_text, all_token_counts
        iter = reduce_token_size(history, chatbot, all_token_counts, top_p, temperature, stream=False, selected_model=selected_model, hidden=True)
        for chatbot, history, status_text, all_token_counts in iter:
